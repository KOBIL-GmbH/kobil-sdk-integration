...
import sys
import warnings
from .backend import authentication, configuration, BackendError, AST, admin_reference
...
def main(argv=None):
    ...
    parser.add_argument('--target', choices=['ast','idp'], default='ast')
    ...
    try:
        ...
        auth = authentication(cfg); ref = auth['credential'] if args.target=='ast' else admin_reference(cfg['admin'])
        if args.command == 'status':
            ...
            if args.target!='ast': raise ValueError('Use sdk_backend_auth_test for IDP')
            backend = AST(cfg)
            ...
            new.update(schema_version=2, auth={**auth, 'credential':ref if args.target=='ast' else auth['credential']})
            if args.target=='idp':
                new['admin']={k:v for k,v in cfg['admin'].items() if k not in {'credential','password_env'}}
                new['admin']['credential']=ref
